...
(function () {
    ...
    <style>
        ...
        .sidebar { transition: width .3s ease, transform .35s ease; }
        .sidebar.collapsed .sidebar-label,
        .sidebar.collapsed .sidebar-logo-text { opacity: 0; pointer-events: none; }
        .sidebar.collapsed .promo-dropdown-arrow { opacity: 0; }

        #appLayout { margin-left: 16rem; transition: all .3s ease; }
        header { transition: all .3s ease; left: 16rem; width: calc(100% - 16rem); }
        header.sidebar-collapsed { left: 4.5rem !important; width: calc(100% - 4.5rem) !important; }

        #loader { position: absolute; z-index: 10; }
    </style>
</head>
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 text-sm space-y-1">
        ...
        <div x-data="{ open: {{ $isPromoActive ? 'true' : 'false' }} }" class="space-y-1">
            ...
            <button @click="open = !open"
                ...
                {{ $isPromoActive ? 'bg-indigo-600/20 text-indigo-200 ring-1 ring-indigo-600/30 font-semibold'
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <button @click="open = !open"
                    ...
                    <img src="https://i.pravatar.cc/100"
                        class="h-8 w-8 rounded-full border border-gray-600/40">
